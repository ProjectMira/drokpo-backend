...
@router.post("/reports")
def report_user(payload: ReportIn, uid: str = Depends(get_current_uid)):
    reports_service.create_report(uid, payload)
    return {"ok": True}


@router.post("/blocks/{target_uid}")
def block_user(target_uid: str, uid: str = Depends(get_current_uid)):
    reports_service.block_user(uid, target_uid)
    return {"ok": True}


@router.delete("/blocks/{target_uid}")
def unblock_user(target_uid: str, uid: str = Depends(get_current_uid)):
