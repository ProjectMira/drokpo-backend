...
def list_for_user(uid: str) -> list[dict]:
    ...
    # Join the other participant's public profile so the client can render
    # the match list without per-user follow-up requests.
    other_uids = [next((u for u in m.get("users", []) if u != uid), None) for m in matches]
    profiles = users_service.get_public_profiles([u for u in other_uids if u])
    return [
